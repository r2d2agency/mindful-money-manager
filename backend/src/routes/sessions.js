...
router.post("/", async (req, res) => {
  try {
    const { patientId, psychologistId, date, status, paymentStatus, expectedAmount, paidAmount, isRecurring, notes } = req.body;
    const psyId = req.user.role === "psychologist" ? req.user.psychologistId : psychologistId;
    const result = await pool.query(
      "INSERT INTO sessions (patient_id, psychologist_id, date, status, payment_status, expected_amount, paid_amount, is_recurring, notes) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *",
      [patientId, psyId || null, date, status || "scheduled", paymentStatus || "pending", expectedAmount || 0, paidAmount || 0, isRecurring || false, notes || ""]
    );
